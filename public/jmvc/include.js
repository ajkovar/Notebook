 ...
 * </ul>
 * <h2>How to Use</h2>
 * Include can be used independently of JavaScriptMVC; however, this example demonstrates how it is commonly used
 * in JavaScriptMVC apps.  We will:
 * <ol>
 *  <li> Create an application.  </li>
 *  <li> Load the include class and your application from an HTML page.</li>
 *  <li> Include additional JavaScript files into your application.</li>
 *  <li> Concatenate and compress your application's code.</li>
 *  <li> Load the compressed production version of your app.</li>
 * </ol>
 * <h3>Create an application</h3>
 * Create an application by running:
 * @codestart no-highlight
 * js jmvc\generate\app <i>APP_NAME</i>
 * @codeend
 * Example <i>APP_NAME</i>s: jabbify, dls, hrmonitor.
 * <h3>Load include and your app in an HTML page</h3>
 * In your HTML, right before the closing &lt;/body> tag add a script tag like:
 * @codestart no-highlight
 * &lt;script src="<i>PATH/TO/</i>jmvc/include.js?<i>APP_NAME</i>,<i>development</i>" type="text/javascript">
 * &lt;/script>
 * @codeend
 * The parameters after include.js are the <i>application name</i> and the <i>environment</i>.
 * You can also create a page that loads your app by running:
 * @codestart no-highlight
 * js jmvc\generate\page APP_NAME location\to\page.html
 * @codeend
 * If you open your HTML page, you will be able to see that it loads include.js and your application file.
 * <h3>Include JavaScript files</h3>
 * In your application file at <i>'apps\APP_NAME\init.js'</i>, include the files you need 
 * in your project.
 * @codestart
 ...
 *   include.views('views/task/init.ejs') //loads a processed view file
 * })
 * @codeend
 ...
 * <h3>Concat and compress your application</h3>
...
include.fn = include.prototype = {
    ...
    init : function(options){
        if(typeof options == 'function'){
            ...
            this.absolute = pathFile.relative() ? pathFile.join_from(include.getAbsolutePath(), true) : this.path;
    ...
    setSrc : function(){
        if(include.options.debug){
            ...
            (window.console && console.log || print)("   "+parts.join("/"));
        }
...
/* @prototype */
{
    ...
    join_current: function(){
        return this.join_from(include.getPath());
    },
    /**
	...
	normalize: function(){
		...
		var path = this.path;
        if(new File(include.getAbsolutePath()).is_cross_domain() && !this.is_domain_absolute() ){
...
for(var i=0; i<scripts.length; i++) {
	...
	if(src && src.match(/include\.js/)){  //if script has include.js
		var mvc_root = new File( new File(src).join_from( include.pageDir ) ).dir();
...
extend(include,
{
    ...
    setScriptOptions : function(){
        if(include.scriptOptions){
            ...
            if(soParts.length > 1){
                if(!soParts[1].match(/^(?:production|development|test|compress)$/)) 
                    throw "env should be one of: production,development,test,compress";
                include.options.env = soParts[1];
            }
        }
    },
	...
	getAbsolutePath: function(){
		...
		return fwd.relative() ? fwd.join_from(include.root.path, true) : cwd;
	},
