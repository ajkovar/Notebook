 ...
 * <ul>
 *     <li>Controller's [jQuery.Controller.prototype.render render function]</li>
 ...
 * [jQuery.View.Helpers.prototype.partial partial] and [jQuery.View.Helpers.prototype.to_text to_text].
...
jQuery.View = function( options ){
	...
	if(options.view || options.absolute_url || options.view_url){
        ...
        var url = options.absolute_url || 
                  (options.view ? include.root.join( options.view+ (options.view.match(this.extMatch) ? '' : this.ext ) ) : 
                   include.root.join("views/"+options.view_url+ (options.view_url.match(this.extMatch) ? '' : this.ext ))
                   );
        //options.view = options.absolute_url || options.view || options.;
		var template = jQuery.View.get(this.name, this.cache);
		if (template) return template;
	    if (template == jQuery.View.INVALID_PATH) return null;

        try{
            this.text = jQuery.ajax({ url: url+(this.cache || jQuery.browser.rhino ? '' : '?'+Math.random() ),async: false}).responseText;
        }catch(e){}
...
jQuery.View.Helpers.prototype = {
     ...
     * @return {String}
     */
	partial: function(options, data, helpers){
...
include.view = function(path){
	if(include.options.env == 'development' && jQuery.View.cache){
        //should convert path
        ...
        include({path: "../../"+path, process: jQuery.View.process_include, skipInsert: true});
...
jQuery.View.process_include = function(script){
