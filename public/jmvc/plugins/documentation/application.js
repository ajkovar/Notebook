...
/* @prototype */
{
    ...
    addTagToSearchData : function(data, tag, searchData){
        ...
        var letter, l, depth=3, current= searchData;
	...
	addToSearchData : function(list, searchData){
		...
		for(var i =0; i < list.length; i++){
            ...
            searchData.list[fullName] = true; //{name: fullName, className: c.Class.className.toLowerCase(), tag: fullName, title: c.title};
			parts = fullName.split(".");
            for(p=0; p< parts.length; p++){
                part = parts[p].toLowerCase();
                if(part == "jquery") continue;
                this.addTagToSearchData({name: fullName, className: c.Class.className.toLowerCase(), tag: fullName, title: c.title}, part, searchData)
            }
            //now add tags if there are tags
            if(c.tags){
                for(var t=0; t< c.tags.length; t++)
                    this.addTagToSearchData({name: fullName, className: c.Class.className.toLowerCase(), tag: c.tags[t], title: c.title}, c.tags[t], searchData);
            }
    ...
    searchData : function(){
        ...
        new include.File("apps/"+this.name+"/docs/searchData.js").save("_searchData = "+$.toJSON(searchData, false));
