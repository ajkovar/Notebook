...
/* @Prototype */
{
     ...
     search : function(val){
        ...
        this.selected = [];
        $("#left").html({view: "jmvc/plugins/documentation/app/views/results", 
                         data: {list: list, selected: this.selected, hide: false},
                         helpers: DocumentationController.Helpers})
     },
     showDoc : function(docData){
         $("#doc").html({
                            view: "jmvc/plugins/documentation/app/views/"+docData.className.toLowerCase(),
                            data: docData,
                            helpers: DocumentationController.Helpers
                        }).scrollTop(0);
         $("#doc code").highlight();
         ...
         if(api.length){
             var names = [];
             for(var name in _searchData.list){
                 names.push(name)
             }
             $("#api").html(  
     ...
     showResultsAndDoc : function(searchResultsData, docData){
         $("#left").html({view: "jmvc/plugins/documentation/app/views/results",
                                     ...
                                     helpers: DocumentationController.Helpers})
         $("#results").slideDown(function(){$('#results a:first')[0].focus()});
     ...
     show : function(who, data){
        ...
        if(data.children && data.children.length){ //we have a class or constructor
			...
			var list = data.children.sort(Search.sortFn)
			var self = this;
			var results = $("#results");
			if(results.length){
				$("#results").slideUp(
            ...
            if($("#results a").length == 0){
                ...
                $("#left").html({view: "jmvc/plugins/documentation/app/views/results",
                                     data: {list: Search.find("home"), selected: this.selected, hide: false},
                                     helpers: DocumentationController.Helpers})
            }
    ...
    "history.favorites.index subscribe" : function(called, data){
        ...
        //this.render({html: "#left", view: "results",data: {list: list, selected: this.selected, hide: false}});
        $("#left").html({view: "jmvc/plugins/documentation/app/views/results",
                                     data: {list: list, selected: this.selected, hide: false},
                                     helpers: DocumentationController.Helpers})
...
DocumentationController.Helpers = {
    ...
    link : function(content, dontReplace){
        return content.replace(/\[\s*([^\|\]\s]*)\s*\|?\s*([^\]]*)\s*\]/g, function(match, first, n){
            ...
            var url = _searchData.list[first] ? first : null;
            if(url){
                if(!n){
