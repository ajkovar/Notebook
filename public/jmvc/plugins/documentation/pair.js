 ...
 * @hide
 */
jQuery.Class.extend("include.Doc.Pair",
/* @Static */
{
     ...
     * @param {include.Doc.Pair} scope - The current scope of documentation.  
     * This is typically a Class, Constructor, Static, or Prototype
     * @return {include.Doc.Pair} - If a type can be found, the new Doc object; otherwise, null.
     */
    ...
    init : function(){
        if(this.className){
             this._view =  include.Doc.get_template(this.className)    
        }
		this.listing = [];
    },
    /**
     * Adds [include.Doc.Directive|directives] to this class.
     * @codestart
     * {
     *   init: function(){
     *     this._super();
     *     this.add(include.Doc.Directive.Return, include.Doc.Directive.Param)
     *   }
     * }
     * @codeend
     */
...
/* @Prototype */
{
     ...
     * Finally, adds to include.Doc.objects.
     * @param {String} comment
     * @param {String} code
     * @param {include.Doc.Pair} scope
     */
    init : function(comment, code, scope ){
        ...
        if(par){
            include.Doc.objects[this.full_name()] = par.url()+(this.url ? "" : "#"+this.full_name() );
        }
    ...
    linker : function(includeSelf){
        var result = includeSelf ? [ {name: this.full_name(), className : this.Class.className.toLowerCase(), title: this.title, hide: (this.hide ? true: false) }] : [];
        if(this.children){
    ...
    comment_setup: function(){
        ...
        for(var l=0; l < lines.length; l++){
            var line = lines[l];
            var match = line.match(include.Doc.Pair.matchDirective)
            if(match){
